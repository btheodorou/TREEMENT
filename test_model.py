...
# checkpoint = torch.load('./save/model_similarity', map_location=torch.device('cpu'))
...
# pickle.dump(intermediate, open('save/intermediate_similarity.pkl', 'wb'))

intermediate = pickle.load(open('save/intermediate_similarity.pkl', 'rb'))
...
# # Extract and display test metrics
# print("GRANULAR CRITERIA-LEVEL RESULTS:")   

# granular_true_list = []
# granular_pred_list = []
# granular_round_list = []
# for t, p, r, c in zip(new_true_list, new_pred_list, new_round_list, new_criteria_list):
#   if c in granular:
#     granular_true_list.append(t)
#     granular_pred_list.append(p)
#     granular_round_list.append(r)
    
# print('Num Granular: ', len(granular_true_list))

# auroc = metrics.roc_auc_score(granular_true_list, granular_pred_list)
# print('auroc: ', auroc)
# acc = metrics.accuracy_score(granular_true_list, granular_round_list)
# print('acc: ', acc)
# (precisions, recalls, _) = metrics.precision_recall_curve(granular_true_list, granular_pred_list)
# auprc = metrics.auc(recalls, precisions)
# print('prc: ', auprc)
# f1 = metrics.f1_score(granular_true_list, granular_round_list)
# print('f1: ', f1)

# auroc_bootstrap = []
# for _ in tqdm(range(n_boot)):
#   samples = random.choices(list(zip(granular_true_list, granular_pred_list)), k=ndata)
#   auroc_bootstrap.append(metrics.roc_auc_score([i[0] for i in samples], [i[1] for i in samples]))
# auroc_pm = (np.percentile(auroc_bootstrap, 97.5) - np.percentile(auroc_bootstrap, 2.5)) / 2
# print('auroc_pm: ', auroc_pm)

# acc_bootstrap = np.mean(np.random.choice([int(r == t) for (r,t) in zip(granular_round_list, granular_true_list)], size=(n_boot, ndata)), axis=1)
# acc_pm = (np.percentile(acc_bootstrap, 97.5) - np.percentile(acc_bootstrap, 2.5)) / 2
# print('acc_pm: ', acc_pm)

# auprc_bootstrap = []
# for _ in tqdm(range(n_boot)):
#   samples = random.choices(list(zip(granular_true_list, granular_round_list)), k=ndata)
#   (precisions, recalls, _) = metrics.precision_recall_curve([i[0] for i in samples], [i[1] for i in samples])
#   auprc_bootstrap.append(metrics.auc(recalls, precisions))
# auprc_pm = (np.percentile(auprc_bootstrap, 97.5) - np.percentile(auprc_bootstrap, 2.5)) / 2
# print('auprc_pm: ', auprc_pm)
...
# pickle.dump(results, open('save/criteria_granular_results.pkl', 'wb'))
# print("\n\n")
...
for (_,t), v in trial_enrollments.items():
  ...
  if 'rare' in disease_mapping[t]:
    rare.append(v)
  if 'diabetes' in disease_mapping[t]:
    diabetes.append(v)
  if 'ms' in disease_mapping[t]:
    ms.append(v)
  if 'parkinson' in disease_mapping[t]:
    parkinson.append(v)
...
acc_diabetes = np.mean(diabetes)
print('acc_diabetes: ', acc_diabetes)
acc_bootstrap_diabetes = np.mean(np.random.choice(diabetes, size=(n_boot, ndata)), axis=1)
acc_pm_diabetes = (np.percentile(acc_bootstrap_diabetes, 97.5) - np.percentile(acc_bootstrap_diabetes, 2.5)) / 2
print('acc_pm_diabetes: ', acc_pm_diabetes)
acc_ms = np.mean(ms)
print('acc_ms: ', acc_ms)
acc_bootstrap_ms = np.mean(np.random.choice(ms, size=(n_boot, ndata)), axis=1)
acc_pm_ms = (np.percentile(acc_bootstrap_ms, 97.5) - np.percentile(acc_bootstrap_ms, 2.5)) / 2
print('acc_pm_ms: ', acc_pm_ms)
acc_parkinson = np.mean(parkinson)
print('acc_parkinson: ', acc_parkinson)
acc_bootstrap_parkinson = np.mean(np.random.choice(parkinson, size=(n_boot, ndata)), axis=1)
acc_pm_parkinson = (np.percentile(acc_bootstrap_parkinson, 97.5) - np.percentile(acc_bootstrap_parkinson, 2.5)) / 2
print('acc_pm_parkinson: ', acc_pm_parkinson)
...
diabetes_len = len(diabetes)
print('diabetes_len: ', diabetes_len)
ms_len = len(ms)
print('ms_len: ', ms_len)
parkinson_len = len(parkinson)
print('parkinson_len: ', parkinson_len)
