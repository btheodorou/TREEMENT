...
ec_network.load_state_dict(checkpoint['ec'])
ehr_network.load_state_dict(checkpoint['ehr'])
align_network.load_state_dict(checkpoint['align'])
...
id_list = []
trial_list = []
for iteration in tqdm(range(0, len(test_trial_dataset), batch_size)):
  ec_network.eval()
  ehr_network.eval()
  align_network.eval()
  ...
  loss, pred = model.get_loss(batch_criteria, batch_criteria_mask, 
